...
class PolynomialDecaySchedule(FairseqLRScheduler): #
    ...
    def step(self, epoch, val_loss=None):
        ...
        super().step(epoch, val_loss) #Set the best validation loss 
        self.lr = self.get_next_lr(epoch)
        self.optimizer.set_lr(self.warmup_factor * self.lr)
        return self.optimizer.get_lr()

    def step_update(self, num_updates):  # here comes the role of the validation loss

      

        """Update the learning rate after each update."""
        if self.args.warmup_updates > 0 and num_updates <= self.args.warmup_updates:
            self.warmup_factor = num_updates / float(self.args.warmup_updates) #setting the warmup factor in to one once it pass the warmup updates
