...
class Trainer(object):
    ...
    def load_checkpoint(
        ...
        if last_optim_state is not None and not reset_optimizer: #not reset optimizer to start from the where it left
    ...
    def get_train_iterator(self, epoch, combine=True):
        ...
        self.task.load_dataset(self.args.train_subset, epoch=epoch, combine=combine) #dataloader happens in trainer.py

        return self.task.get_batch_iterator(  #get_batch_iterator fairseq_task.py  Get an iterator that yields batches of data from the given dataset.
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        self._set_seed()
        self.model.train() #Can add the other models to here  make them eval
        ...
        for i, sample in enumerate(samples):
            sample = self._prepare_sample(sample)   #After getting this sample we can send the sample via pre trained models to extract features
            if sample is None:
            ...
            try:
                with maybe_no_sync(): #Training Starts
    ...
    def lr_step(self, epoch, val_loss=None):  #LR sheduler
