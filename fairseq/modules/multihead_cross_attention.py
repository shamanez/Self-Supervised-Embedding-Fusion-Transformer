...
class MultiheadCrossAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads,qdim=None, kdim=None, vdim=None, dropout=0., bias=True,
        ...
        self.qkv_same_dim = self.kdim == embed_dim and self.vdim == embed_dim and self.qdim==embed_dim
        ...
        if self.qkv_same_dim:
            exit("Wrong --- Cannot get the self attetnion - multihead_cross_attention.py")
            self.in_proj_weight = Parameter(torch.Tensor(3 * embed_dim, embed_dim))
        else:
            #Projecting to the QKV from the inputs
