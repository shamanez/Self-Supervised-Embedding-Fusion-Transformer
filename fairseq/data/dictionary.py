...
class Dictionary(object):
    ...
    @classmethod #Interpreter can access to class methods without an objext
    ...
    def add_from_file(self, f, ignore_utf_errors=False):
        ...
        for line in lines[indices_start_line:]:
            idx = line.rfind(' ')  #Find what is the index which seperate the word
            if idx == -1:
                raise ValueError("Incorrect dictionary format, expected '<token> <cnt>'")
            word = line[:idx] # word index
            count = int(line[idx + 1:]) #Count of that given word
