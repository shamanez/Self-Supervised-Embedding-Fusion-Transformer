...
from fairseq.models.vid2vec import ConvFeatureExtractionModel
...
def load_checkpoint(args, trainer):
    ...
    if extra_state is not None and not args.reset_dataloader:
        ...
        epoch_itr = trainer.get_train_iterator(epoch=0) #data_loader
...
def load_external_checkpoint_to_cpu(path, arg_overrides=None):
    """Loads a checkpoint to CPU (with upgrading for backward compatibility)."""
    state = torch.load(
        path, map_location=lambda s, l: default_restore_location(s, 'cpu'),
    )
...
def load_model_external(filenames, arg_overrides=None, task=None):
    """Loads an ensemble of models.
    ...
    Args:
        ...
        task (fairseq.tasks.FairseqTask, optional): task to use for loading
    """
    ensemble, _task = load_model_external_and_task(filenames, arg_overrides, task)
...
def load_model_external_and_task(filenames, arg_overrides=None, task=None):
    from fairseq import tasks
    ...
    for filename in filenames:
        if not os.path.exists(filename):
            raise IOError('Model file not found: {}'.format(filename))
        state = load_external_checkpoint_to_cpu(filename, arg_overrides)


        #State Dict of the pretrained chekpoint
        checkpoint_state_dict=state['state_dict_model']
    
        inner_nc = 256; num_classes = 8; num_additional_ids=32
        model_video=ConvFeatureExtractionModel(3, inner_nc=inner_nc, num_additional_ids=num_additional_ids)

        current_model_state_dict=model_video.state_dict() #Getting the set of current paramerter dict

        common_state_dict = {k: v for k, v in checkpoint_state_dict.items() if k in current_model_state_dict} 
      
        current_model_state_dict.update(common_state_dict)

        model_video.load_state_dict(current_model_state_dict, strict=True)

        ensemble.append(model_video)
